  ...
  <tbody>
    ...
    <tr>
      ...
      <td>フロントエンドは Nuxt.js(Vue.js)、 サーバサイドは Laravel を用いて転職サイトの新規立ち上げからリリース、保守/運用を担当。
        <details>
          <summary>詳細</summary>
          サーバーサイドはAPIの設計/実装から、フロントはAtomic Designを用いたコンポーネント設計/実装まで多岐に渡り開発に従事しました。新規事業ということもあり、如何に早く作りリリースすることができるかを心掛けました。ビジネスサイドとも密にコミュニケーションを取り、優先順位をどうするか何度も協議を重ねたり、一部開発チームメンバーのディレクションを行なったりと幅広い業務に従事してきました。スピード重視にした結果、LaravelのControllerが肥大化してしまった負債を作ってしまい、リリース後はMVCからADR(Action Domain Responder)にリファクタリングの対応も行いました。
        </details>
      </td>
    </tr>
  </tbody>
